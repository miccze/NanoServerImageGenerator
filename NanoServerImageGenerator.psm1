...
Function New-NanoServerImage
{
    ...
    Process
    {
        ...
        $OptionalFeatures = [String[]](Initialize-OptionalFeature -OptionalFeature $SetupUI)  # miccze - Initialize-OptionalFeature is clean
        ...
        try
        {
            ...
            # Phase 1 ---- miccze --- ZAkomentowanie kopiowania plików do $script:basepath, $Script:BasePackagesPath, $Script:BaseLanguagesPackagesPath
            #Copy-Files
            
            if ($OptionalFeatures)
            {
                Mount-Image $Script:ImageFilePath $Script:Editions[$Edition] -Readonly  #miccze - Zmiana $sript:BaseWimImageFilePath na $Script:ImageFilePath
...
Function Get-MediaOptionalFeature()
{
    ...
    $DismCapabilities = Invoke-Dism "/Image:'$Script:TargetMountPath' /Get-Capabilities /Source:'$Script:PackagesPath'" -ReturnOutput -FullOutput # micccze - zmiana $Script:BasePackagesPath na $Script:PackagesPath
    $Capabilities = @{}
    ...
    [String]$LogsPath)
{
    ...
    {
        $Packages | ForEach-Object {
            #miccze -below. zmiana $Script:BasePackagesPath na $Script:PackagesPath 
            $Script:BasePackageFilePaths.Add($_, (Join-Path -Path $Script:PackagesPath -ChildPath (Get-PackageFileName $_))) 
            # miccze - below. zmiana $Script:BaseLanguagePackagesPath na $Script:LanguagePackagesPath 
            $Script:BaseLanguagePackageFilePaths.Add($_, (Join-Path -Path $Script:LanguagePackagesPath -ChildPath (Get-PackageFileName $_ $Script:Language))) 
        }
    }
    #miccze - Odpowiednie zadeklarowanie zmiennej $Script:TargetImageFilePath. 
    #Zmienna $Script:TargetImageFilePath musi mieć format obrazu do utworzenia na końcu, tak jak wczesniej miała $Script:BaseImageFilePath
    #czyli musi konczyc sie formatem obrazu (vhdx lub vhd)
    if ($TargetPath)
    {
...
Function Test-Paths([String[]]$Packages, [String[]]$ServicingPackages, [String]$UnattendPath)
{
    ...
    else
    {
        ...
        # Check that the Nano Server image is present in the base path. --- miccze --- zakomentowanie tego, jako że BaseWimImageFilePath bedzie zastąpione $Script:ImageFilePath
...
Function Copy-Files()  # miccze - This function is not called, i commented it in New-NanoServerImage Function. Line 639 
{
    if (!$Script:HasMediaPath)
    {
...
Function Export-WimImage([String]$Edition)
{
    ...
    $cmd = ($cmdTemplate -f $Script:ImageFilePath, $Script:Editions[$Edition], $Script:TargetImageFilePath) #miccze - Changing $Script:BaseWimImageFilePath na $Script:ImageFilePath, and changing $Script:BaseImageFilePath na $Script:TargetImageFilePath
    Invoke-Dism $cmd
}
...
{
    $Parameters = @{}
    $Parameters["-SourcePath"] = $Script:ImageFilePath # miccze - zmiana źródła z $Script:BaseWimImagFilePath na $Script:ImageFilePath
    $Parameters["-VHDPath"] = $Script:TargetImageFilePath #miccze - zmiana folderu docelowego z $Script:BaseImageFilePath na $Script:TargetImageFilePath
...
Function Convert-Image([String]$Edition)
{
    ...
    if (-not (Test-Path $Script:TargetImageFilePath))  #miccze- zmiana folderu do testowania z $Script:BaseImageFilePath na $Script:TargetImageFilePath
    {
        Throw $Strings.ERR_IMAGE_WAS_NOT_PRODUCED
    }
    
    #Move-Item -Path $Script:BaseImageFilePath -Destination $Script:TargetImageFilePath -Force   #miccze --- commenting that as the image will created already in $Script:TargetImageFilePath
...
Function Add-OptionalFeatures([String[]]$OptionalFeature)
{
    ...
    $OptionalFeature | ForEach-Object { 
        ...
        Add-OptionalFeature $Identity $Script:PackagesPath # miccze - zmiana z $Script:BasePackagesPath na $Script:PackagesPath
        Add-OptionalFeature $Identity $Script:LanguagePackagesPath -Language #miccze - zmiana z $Script:BaseLanguagePackagesPath na $Script:LanguagePackagesPath
